...
def interpolate_body_movement(spy, curr_qpos, dx, dy, progress):
    """
    ...
    """
    # Use the in_body_inv_kinem function with the delta movement
    # Multiply by progress for smooth interpolation
    target_ctrl = spy.in_body_inv_kinem(dx * progress, dy * progress, 0, curr_qpos, to_print=False)
    return np.array(target_ctrl).flatten()
        ...
        if elapsed_time < 2.0:
            ...
            if phase_type == 'leg':
                ...
                target_ctrl = interpolate_body_movement(
                    spy, initial_qpos_for_phase, dx, dy, progress
                )
            ...
            if progress >= 1.0 and np.all(np.abs(data.qpos[7:] - target_ctrl) < 0.05):
